...
const Home = () => {
  ...
  useEffect(()=>{
    const fetchRecent = async () => {
      const response = await fetch(`/api/listing/search?sort=createdAt&order=desc`)
      const data = await response.json()
      setRecent(data.lists)
    }
  ...
  return (
    ...
    <Helmet title='Real-estate'/>
      ...
      <div className='w-full overflow-hidden'>
        ...
        <Slider {...settings}>
            {recent && 
              recent.slice(0, 3).map( offer => (
                offer.images.map((recentImage,i)=> (
                  <div key={i}>
                  <img
